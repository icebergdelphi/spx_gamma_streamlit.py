...
if (current_time - st.session_state['last_request_time']).total_seconds() > 300 or st.session_state['data'] is None:
    ...
    selection = st.session_state.get('selection', 'SPX')  # Usar la última selección si existe
    risk_free_rate = st.session_state.get('risk_free_rate', 4.0)  # Usar la última tasa si existe
...
if data_json:
    ...
    if "SPX" in selection or "NDX" in selection:
    ...
    elif "SPX=>ES" in selection or "NDX=>NQ" in selection:
        # Gráfico para mostrar la conversión SPX=>ES o NDX=>NQ
        future_label = "ES" if "SPX" in selection else "NQ"
        index_label = "SPX" if "SPX" in selection else "NDX"

        # Crear un gráfico de comparación
        fig3 = go.Figure()
        fig3.add_trace(go.Scatter(
            x=[index_label, future_label],
            y=[spotPrice, futurePrice],
            mode='lines+markers',
        ...
        fig3.update_layout(
            title=f"{index_label} to {future_label} Conversion ({index_label}={spotPrice:,.0f}, {future_label}={futurePrice:,.2f})",
            xaxis_title="Instrument",
            yaxis_title="Price",
            yaxis=dict(range=[spotPrice * 0.95, futurePrice * 1.05], tickformat=","),
            showlegend=True,
            template="plotly_dark",
            font=dict(size=14),
            margin=dict(l=20, r=20, t=50, b=50),
        ...
        price_diff = futurePrice - spotPrice
        st.write(f"Diferencia ({future_label} - {index_label}): {price_diff:,.2f} puntos")
